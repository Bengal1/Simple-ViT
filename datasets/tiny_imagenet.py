...
class TinyImageNetDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            root (str):
                Directory containing or intended to contain the
                ``tiny-imagenet-200`` dataset.
            split (str):
                Dataset split to load. Supported values:
        ...
        Raises:
            ValueError:
                If `split` is not one of the supported options.
        """
        if split not in {"train", "val", "labeled", "test"}:
            raise ValueError(
                "split must be one of: 'train', 'val', 'labeled', 'test'"
            )
    ...
    def _load_classes(self) -> None:
        ...
        This ensures a consistent class-to-index mapping across all dataset
        splits.

        Raises:
            FileNotFoundError:
                If ``wnids.txt`` does not exist.
        """
        ...
        self.idx_to_class = {
            idx: wnid for wnid, idx in self.class_to_idx.items()
        }

    def _load_samples(self) -> None:
        """
        Load dataset samples according to the selected split.
        """
    ...
    def _load_train(self) -> None:
        ...
        Raises:
            FileNotFoundError:
                If the training directory does not exist.
        """
    ...
    def _load_val(self) -> None:
        """
        Load labeled samples from the validation split using the annotation
        file.

        Raises:
            FileNotFoundError:
                If the validation image directory or annotation file does not
                exist.
        """
    ...
    def __getitem__(self, index: int):
        ...
        Returns:
            tuple:
                The loaded image (optionally transformed) and its label.
...
def _download_tiny_imagenet(
    ...
    Args:
        ...
        url (str):
            URL to the Tiny ImageNet archive.
